...
import 'package:instagram_clone_coding/pages/search/search_focus.dart';
...
class _SearchScreenState extends State<SearchScreen>
  ...
  List<List<int>> groupBox = [
    ...
    [],
    []
  ]; //3개의 컬럼에 있는 리스트별로 사이즈가 들어간다. (1, 2) 중에 하나로. // QnADartGrammar 2차배열
  List<int> groupIndex = [
    0,
    0,
    0
  ]; // 전체 사이즈 합계를 체크하기 위한 리스트 // 라이브러리를 추가한다.quiver, 각 컬럼별 사이즈가 합계된다.
  ...
  void initState() {
    ...
    super.initState();
    for (var i = 0; i < 100; i++) {
      var gi = groupIndex.indexOf(min<int>(
          groupIndex)!); // 여기 봐라. 0,1,2 중에서 제일 작은거 찾아서 인덱스(0,1,2) 를 넘겨줌.
      //var gi = i % 3;
      var size = 1;
      if (gi != 1) {
        // 그러니깐 중간값 1이 아니면
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SafeArea(
        // AppBar 를 안쓰면서 AppBar 처럼 작업하면 상태표시줄을 넘어가기때문에 SafeArea 위젯을 사용한다.
        child: Column(
          children: [
            _myAppBarInColumn(),
    ...
    return SingleChildScrollView(
      child: Row(
        ...
        children: List.generate(
          ...
          (index) => Expanded(
            child: Column(
              children: // 총 3개의 확장된 컬럼이 들어가는거고..
                  List.generate(
                      groupBox[index].length,
                      (jndex) => Container(
                            height: Get.width * 0.33 * groupBox[index][jndex],
                            // groupBox[index][jndex] 의 값은 1 이나 2 가 들어간다.
...
*/
  ...
  Widget _myAppBarInAppBar() {
    return AppBar(
      // 컬럼 안에도 AppBar 를 리턴할 수 있다는거지.. 잘 기억하자.
      title: Container(
    ...
    children: [
      Expanded(
        child: GestureDetector(
          onTap: () {
            Get.to(SearchFocus(), id: 1); // 이거 안에서 움직여라는 뜻 왜냐하면 벌써 들어와 있으니깐...
            // 만약에 id 를 넣지 않으면 현재 Navigator 가 상단으로 올라가버리므로 BottomNavigationBar 가
            // 보이지 않게 되지..
          },
          child: Container(
